	...
	if(fmt == FMT_IGES)
     {
	  ...
	  std::cout << reader.NbShapes() << " shapes read from input file" << std::endl;
...
void OCC_STEP_MESHER::meshShape ( TopoDS_Shape& theShape )
{
  ...
  bool isTopLabelNull = top_label.IsNull() == Standard_True;
  bool isTopLabelHasChild = top_label.HasChild();


   if((!isTopLabelNull) && isTopLabelHasChild)
   {
